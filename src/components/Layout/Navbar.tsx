  ...
  return (
    <div className="flex items-center justify-between shadow-lg px-[120px]">
      ...
      <div className="flex items-center justify-between w-1/6">
        ...
        >
          <div
            className="flex bg-white rounded-full items-center px-6 py-4"
            style={{ width: "500px" }}
            onClick={(e) => {
              e.stopPropagation();
            }}
        ...
        <FaRegUser
          className="text-2xl cursor-pointer"
          onClick={() =>
            navigate(isLoggedIn ? `/profile/${user.id}` : defaultPage)
          }
        />
        <FaRegHeart
          className="text-2xl cursor-pointer"
      ...
      <div
        className={`bg-[rgba(0,0,0,0.1)]  z-50 fixed top-0 right-0 left-0 bottom-0 flex flex-col ${
          !models.cart && "hidden"
        } cursor-default`}
